...
local radar =
{
	shown = false,
	freeze = false,
	info = "~y~Iniciando Radar...~w~3 2 1...~y~Carregado! ",
	info2 = "~y~Iniciando Radar...~w~3 2 1...~y~Carregado! ",
	minSpeed = 5.0,
	maxSpeed = 75.0,
}
...
Citizen.CreateThread( function()
	...
	while true do
		...
		if IsControlJustPressed(1, 128) and IsPedInAnyPoliceVehicle(GetPlayerPed(-1)) then
			...
			if radar.shown then 
				radar.shown = false 
				radar.info = string.format("~y~Iniciando Radar...~w~3 2 1...~y~Carregado! ")
				radar.info2 = string.format("~y~Iniciando Radar...~w~3 2 1...~y~Carregado! ")
			else 
				radar.shown = true 
			end		
                Wait(75)
		...
		if radar.shown  then
			if radar.freeze == false then
					...
					if IsEntityAVehicle(e) then
						...
						radar.info = string.format("~y~Placa: ~w~%s  ~y~Modelo: ~w~%s  ~y~Velocidade: ~w~%s kph", fplate, fmodel, math.ceil(fvspeed))
					end
					...
					if IsEntityAVehicle(j) then
						...
						radar.info2 = string.format("~y~Placa: ~w~%s  ~y~Modelo: ~w~%s  ~y~Velocidade: ~w~%s kph", bplate, bmodel, math.ceil(bvspeed))
			...
			DrawAdvancedText(0.591, 0.903, 0.005, 0.0028, 0.4, "Frente Radar", 0, 191, 255, 255, 6, 0)
			DrawAdvancedText(0.591, 0.953, 0.005, 0.0028, 0.4, "Trás Radar", 0, 191, 255, 255, 6, 0)
