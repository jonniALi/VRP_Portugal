...
-- versioning
print("[ice41] launch version "..version)
--[[
PerformHttpRequest("https://raw.githubusercontent.com/ImagicTheCat/vRP/master/vrp/version.lua",function(err,text,headers)
  if err == 0 then
    ...
    if version ~= r_version then
      print("[ice41] WARNING: A new version of vRP is available here https://github.com/ImagicTheCat/vRP, update to benefit from the last features and to fix exploits/bugs.")
    end
  else
    print("[ice41] unable to check the remote version")
  end
end, "GET", "")
--]]
...
AddEventHandler("playerConnecting",function(name,setMessage, deferrals)
  ...
  if ids ~= nil and #ids > 0 then
    deferrals.update("[ice41] Checking identifiers...")
    vRP.getUserIdByIdentifiers(ids, function(user_id)
      ...
      if user_id ~= nil then -- check user validity 
        deferrals.update("[ice41] Checking banned...")
        vRP.isBanned(user_id, function(banned)
          if not banned then
            deferrals.update("[ice41] Checking whitelisted...")
            vRP.isWhitelisted(user_id, function(whitelisted)
              if not config.whitelist or whitelisted then
                ...
                if vRP.rusers[user_id] == nil then -- not present on the server, init
                  ...
                  deferrals.update("[ice41] Loading datatable...")
                  vRP.getUData(user_id, "vRP:datatable", function(sdata)
                    ...
                    deferrals.update("[ice41] Getting last login...")
                    vRP.getLastLogin(user_id, function(last_login)
                      ...
                      -- trigger join
                      print("[ice41] "..name.." ("..vRP.getPlayerEndpoint(source)..") joined (user_id = "..user_id..")")
                      TriggerEvent("vRP:playerJoin", user_id, source, name, tmpdata.last_login)
                      deferrals.done()
                    end)
                  end)
                else -- already connected
                  print("[ice41] "..name.." ("..vRP.getPlayerEndpoint(source)..") re-joined (user_id = "..user_id..")")
                ...
                print("[ice41] "..name.." ("..vRP.getPlayerEndpoint(source)..") rejected: not whitelisted (user_id = "..user_id..")")
                deferrals.done("[ice41] Não está na whitelist envie o seu id no discord (user_id = "..user_id..") discord.gg/.")
              end
            end)
          else
            print("[ice41] "..name.." ("..vRP.getPlayerEndpoint(source)..") rejected: banned (user_id = "..user_id..")")
            deferrals.done("[ice41] Banido (user_id = "..user_id..").")
          end
        end)
      else
        print("[ice41] "..name.." ("..vRP.getPlayerEndpoint(source)..") rejected: identification error")
        deferrals.done("[ice41] Identification error.")
      end
    end)
  else
    print("[ice41] "..name.." ("..vRP.getPlayerEndpoint(source)..") rejected: missing identifiers")
    deferrals.done("[ice41] Missing identifiers.")
  end
  Debug.pend()
end)
...
AddEventHandler("playerDropped",function(reason)
  ...
  if user_id ~= nil then
    ...
    print("[ice41] "..vRP.getPlayerEndpoint(source).." disconnected (user_id = "..user_id..")")
