...
function StartJob(jobid)
	if jobid == 1 then -- taxi
		showLoadingPromt("Carregando trabalho: Condutor de Uber", 2000, 3)
		...
		Wait(2000)
		DrawMissionText("Dirija e procure ~h~~y~passageiros~w~.", 10000)
		onJob = jobid
	end
end
				...
				if IsPedSittingInVehicle(GetPlayerPed(-1), jobs.cars[1]) then
					if DoesEntityExist(jobs.peds[1]) then
						if IsPedFatallyInjured(jobs.peds[1]) then
							...
							DrawMissionText("Seu cliente está  ~r~morto~w~. Encontre outro.", 5000)
						else
							if jobs.flag[1] == 1 and jobs.flag[2] > 0 then
								...
								if jobs.flag[2] == 0 then
									...
									Citizen.InvokeNative(0xB736A491E64A32CF,Citizen.PointerValueIntInitialized(jobs.peds[1]))
									jobs.peds[1] = nil
									DrawMissionText("Cliente ~r~Cansado de esperar~w~. Encontre outro.", 5000)
									jobs.flag[1] = 0
									jobs.flag[2] = 59+GetRandomIntInRange(1, 61)
								else
									if IsPedSittingInVehicle(GetPlayerPed(-1), jobs.cars[1]) then
							...
							if jobs.flag[1] == 2 and jobs.flag[2] > 0 then
								...
								if jobs.flag[2] == 0 then
									...
									Citizen.InvokeNative(0xB736A491E64A32CF,Citizen.PointerValueIntInitialized(jobs.peds[1]))
									jobs.peds[1] = nil
									DrawMissionText("~r~Cliente não vai com você~w~. Encontre outro.", 5000)
									jobs.flag[1] = 0
									jobs.flag[2] = 59+GetRandomIntInRange(1, 61)
								else
									if IsPedSittingInVehicle(jobs.peds[1], jobs.cars[1]) then
										...
										if street[2] ~= 0 and street[2] ~= nil then
											local streetname = string.format("~c~Leve-me para o %s, próximo %s", GetStreetNameFromHashKey(street[1]),GetStreetNameFromHashKey(street[2]))
											DrawMissionText(streetname, 5000)
										else
											local streetname = string.format("~c~Leve-me para o %s", GetStreetNameFromHashKey(street[1]))
											DrawMissionText(streetname, 5000)
										end
							...
							if jobs.flag[1] == 3 then
								if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), jobs.coords.cx[jobs.flag[2]],jobs.coords.cy[jobs.flag[2]],jobs.coords.cz[jobs.flag[2]], true) > 4.0001 then
									...
									jobs.peds[1] = nil
									Wait(6000)
									DrawMissionText("~g~Entregou o cliente!", 5000)
						...
						if jobs.flag[1] == 0 and jobs.flag[2] > 0 then
							...
							if jobs.flag[2] == 0 then
								...
								if DoesEntityExist(rped) then
									...
									DrawMissionText("~g~Cliente~w~ esperar-o. Dirija nas proximidades", 5000)
									...
									jobs.flag[1] = 0
									jobs.flag[2] = 59+GetRandomIntInRange(1, 61)
									DrawMissionText("Dirija e procure ~h~~y~clientes~w~.", 10000)
								end
							end
						end
					end
					...
					if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), GetEntityCoords(jobs.cars[1]), true) > 30.0001 then
						...
						DrawMissionText("Volte no seu carro para continuar. Ou vá embora para parar de trabalhar.", 1)
					end
				end
			else
				StopJob(1)
				DrawMissionText("O táxi está ~h~~r~destruído~w~.", 5000)
			end
		end
	end
end)