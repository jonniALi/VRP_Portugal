...
function vRPbm.spawnVehicle(model) 
    ...
    while not HasModelLoaded(mhash) and i < 1000 do
	  if math.fmod(i,100) == 0 then
	    vRP.notify({"~b~Carregando o modelo do veículo."})
	  end
      RequestModel(mhash)
      Citizen.Wait(30)
	  i = i + 1
    ...
    if HasModelLoaded(mhash) then
	  ...
	  vRP.notify({"~g~Veículo gerado."})
	else
	  vRP.notify({"~r~Modelo do veículo inválido."})
	end
end
