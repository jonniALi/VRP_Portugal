        ...
        if ( IsPedSittingInAnyVehicle( ped ) ) then 
            ...
            if ( GetPedInVehicleSeat( vehicle, -1 ) == ped ) then 
                ...
                if ( DoesEntityExist( vehicle ) ) then 
                	ShowNotification( "~r~Não foi possível excluir o veículo, tente novamente." )
                else 
                	ShowNotification( "Veículo excluído." )
                end 
            else 
                ShowNotification( "Deve estar no banco do motorista!" )
            end 
        else
            ...
            if ( DoesEntityExist( vehicle ) ) then 
                ...
                ShowNotification( "Deve estar dentro ou perto de um veículo para excluí-lo." )
            end 
        end 
    end 
end )
