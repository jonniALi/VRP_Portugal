	...
	while true do
		...
		if showHelp then
			if helpTimer > GetGameTimer() then
				showHelpText("Navegue com ~y~W,A,S,D~s~ e confirme com ~y~SPAÇO~s~ para o bloco de código esquerdo.")
			elseif helpTimer > GetGameTimer()-helpCycle then
				showHelpText("Use as ~y~teclas das Setas~s~ e ~y~ENTER~s~ para o bloco de código direito")
			else
